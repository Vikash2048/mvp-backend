...
const verifyOTP = catchAsync(async(req, res, next) => {
  ...
  const { accessToken, refreshToken, userObj } = await userService.verifyOTP(req.body);
...
const refreshedAccessToken = catchAsync(async(req, res, next) => {
  ...
  const accessToken = await jwtService.authenticateRefreshToken(refreshToken);
...
const getAvatars = async (req, res) => {
  try {
    ...
    const normalizedAvatars = avatars.map((avatar) => ({
      ...avatar.toObject(),
      url: normalizeAvatarUrl(avatar.url),
    }));

    return res.json({
      data: normalizedAvatars,
      status: 200,
      message: "Avatars fetched successfully",
    });
  } catch (error) {
    logger.failure("Fetching avatars failed", error, {
      method: req.method,
      endpoint: `${req.method} ${req.originalUrl}`,
      ip: req.ip,
    });
...
export { sendOTP, verifyOTP, refreshedAccessToken,  getAvatars, logout,updateUserProfile, getUserProfile, updateStreak };