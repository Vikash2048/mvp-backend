...
/* GET ALL THERAPIST */
export const getTherapists = catchAsync(async(req, res) => {
  req.log.info("Get therapist request received");
  const data = await therapistService.getTherapists(req.query);
  res.status(200).json({success: true, ...data});
});

/* GET SINGLE THERAPIST */
export const getTherapistById = catchAsync(async(req, res) => {
  req.log.info("Get therapist by id request received");
  const data = await therapistService.getTherapistById(req.params.id);
  res.status(200).json({success: true, data});
});

/* CREATE THERAPIST */
export const createTherapist = catchAsync(async(req, res) => {
  req.log.info("Create therapist request received");
  const data = await therapistService.createTherapist(req.body);
  res.status(201).json({success: true, message: "Therapist created successfully"});
});

/* UPDATE THERAPIST */
export const updateTherapist = catchAsync(async(req, res) => {
  req.log.info("Update therapist request received");
  const data = await therapistService.updateTherapist(req.params.id, req.body);
  res.status(200).json({success: true, message: "Therapist updated successfully"});
});

/* GET THERAPIST AVAILABILITY */
export const getTherapistAvailability = catchAsync(async(req, res) => {
  req.log.info("Get Therapist availability request received");
  const data = await therapistService.getTherapistAvailability(req.params.id, req.query);
  res.status(200).json({success: true, data});
});
